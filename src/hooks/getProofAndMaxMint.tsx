...
import { snapshots, snapshotType } from '../data/snapshot';
...
export default function useGetProofAndMaxMint() {
  ...
  const getProof = (): { error: boolean; proof: `0x${string}`[]; maxMint: number } => {
    if (address) {
      const found = snapshots.find((snapshot: snapshotType) => snapshot.address === address);
  ...
  const generateMerkleRoot = (): string => {
    const list = snapshots.map((snapshot: snapshotType) => {
...
function encodeLeaf(address: `0x${string}`, spots: number) {
...
function generateProof(address: `0x${string}`, maxAmount: number) {
  const list = snapshots.map((snapshot: snapshotType) => {
    return encodeLeaf(snapshot.address, snapshot.maxMint)
  })
  const merkleTree = new MerkleTree(list, keccak256, {
